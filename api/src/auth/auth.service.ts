...
export class AuthService {
  ...
  async login(loginDto: LoginDto): Promise<{ id: string, token: string, role: string }> {
    ...
    // Create the JWT payload with the user email, id, and role
    const payload = { email: user.email, sub: user._id, role: user.role };  // Assuming user.role exists
    const token = this.jwtService.sign(payload);
  
    // Return user id, token, and role
