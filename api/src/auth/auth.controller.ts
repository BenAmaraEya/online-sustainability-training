...
export class AuthController {
  ...
  @Post('/login')
  async login(@Body() loginDto: LoginDto, @Session() session: Record<string, any>): Promise<{ id: string, token: string, role: string }> {
    const user = await this.authService.login(loginDto);
    session.token = user.token;
    session.userId = user.id;
  
    // Store the role in the session (optional, depends on usage)
